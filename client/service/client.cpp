...
void client::createNewUser()
{
	...
	do
	{
		...
		do
		{
			...
			cout << "Enter username: ";
			getline(cin, s);
			for (int i = 0; i < strlen(strToChar(s)); ++i)
			{
				if(s[i]==' '){ 
					checkSpace = true;
					//break;
			...
			if(checkSpace) { cout<< "\tCan not use space in your username! Please enter again!"<<endl; continue; }
			if (s.size()>SIZE_U)
			{
				cout << "\tOut size of username ! Please enter again !" << endl;
				continue;
		...
		while ((s.size()>SIZE_U)||(checkSpace));
		...
		do
		{
			...
			if(checkSpace) { cout<< "\tCan not use space in your password! Please enter again!"<<endl; continue; }
			if (s.size()>SIZE_P)
			{
				cout << "\tOut size of password! Please enter again !"<< endl;
				continue;
		...
		while ((s.size()>SIZE_P)||(checkSpace));
		showAllRoles();
		pthread_cond_wait(&cond, &mutex);
		do
		{
			...
			if(strcmp(strToChar(s),"?ignore")==0)return;
			role =s[0] -'0';
			if(role!=1&&role!=2) cout<<"Out of rule! Please enter again!"<<endl;
...
void client::modifyRoomHandler()
{
	...
	while(getline(std::cin, str_newCapacity))	// input data as a string
    {
			stringstream temp(str_newCapacity); // Convert to string stream
			if (temp >> int_newCapacity)		// if the string is only number then assign to the integer argument
			{
				if (temp.eof())					// to the end of string is success
				{  
					break;
				}
			}
	...
	pkRoomModify.setRoomM(strToChar(roomID_Modify), int_newCapacity);
